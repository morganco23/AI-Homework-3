...
class AIPlayer(Player):
  ...
  def expandNode(self, node, myID):
      ...
      return sorted_node_list[0: math.ceil(0.2 * len(nodeList))]
  ...
  def getBestMove(self, rootNode, myID):
      ...
      for node in possible_moves:
           ...
           """
           if node_utility == None:
               continue
           """
           if rootNode["parent"] == None:
               print("Utility: " + str(best_move_utility))
               if rootNode['min'] != None:
                   print("Min: " + str(rootNode['min']))
           for level in range(rootNode["depth"]):
               print("\t", end="")
           if rootNode["max_not_min"] and rootNode['min'] == None:
               ...
               if rootNode["parent"] == None:
                   print("Replaced with " + str(best_move_utility))
               rootNode["min"] = best_move_utility
               rootNode["bestMove"] = node["move"]
